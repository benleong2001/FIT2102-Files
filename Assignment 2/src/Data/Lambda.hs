...
type ShowLam =
  Bool -> -- Flag if this term is the head term of a chain of applications.
    ...
    where
      ...
      recVar _ i _ _ _ vars = (False, showChar var)
        where
          ...
          errStr =
            error
              ( "No variable exists with index `"
                  ++ show i
                  ++ "` indexes only go up to `"
                  ++ show (length vars - 1)
                  ++ "`"
              )
      ...
      recAp (LamAp (LamShow f) n) _ n' isHead isTail absDepth vars =
        maybe
          ...
          $ f
          $ normal n
      recAp _ m n isHead isTail _ vars =
        let (_, m') = m True False 0 vars
            (_, n') = n False isTail 0 vars
            ss = m' . n'
         in (False, if isHead then ss else open . ss . close)
      recAbs :: Lambda -> Char -> ShowLam -> ShowLam
      recAbs _ c e _ isTail absDepth vars =
        let -- \| Find a free variable name.
            altC =
              fromMaybe (error "exhausted all variable names") $
                find (`notElem` vars) $
                  ['a' .. 'z'] ++ ['A' .. 'Z']
...
recLambda ::
  forall a.
  (Lambda -> Int -> a) -> -- The eliminator for lambda variables.
...
betaNormalise (LamAp m@(LamShow _) n) =
  let f n'@(LamAp (LamShow _) _) = n'
      f n' = LamAp m n'
   in f <$> betaNormalise n
betaNormalise (LamAp m n) =
  let m' = listToMaybe $ betaNormalise m
      ns = betaNormalise n
   in maybe
...
type EtaLam =
  Int -> -- The index of the argument being eta reduced.
...
betaNormal (LamAp (LamAbs _ body) expr) =
  betaNormal $
    recLambda recVar recAp recAbs recShow body 0
  where
    -- \| Cache the fixed Lambdas.
...
etaNormal l@(LamAbs _ (LamAp expr (LamVar 0))) =
  maybe l etaNormal $
...
lamToShowS l = snd <$> recLambda recVar recAp recAbs recShow l True True 0 []
  where
    ...
    recAbs _ c e _ isTail absDepth vars = do
      ...
      let vars' = c' : vars
      (isAbs, e') <- e True True (absDepth + 1) vars'
      let -- \| Shows all of the immediate context along with the body.
          altBody =
            foldl'
              (flip $ (.) . showChar)
              (showChar c')
