describe("exercise_0_suite", function () {
  it("Generated code exists", function () {
    try {
      ...
      throw new Error(
        "Generated code doesn't seem to exist!  Did you run tsc?"
      );
    }
  });
});
...
describe("exercise_3_suite", function () {
  describe("fromArray(array)", function () {
    ...
    it("fromArray turns [1,2,3] into cons(1, cons(2, cons(3, null)))", function () {
      ...
      expect(
        head(list123),
        "If you're getting a 3 you might be reversing the list by mistake."
      ).to.equal(1);
  ...
  describe("reduce(function, initialValue, list)", function () {
    ...
    it("The order of reduce is important. The initial value should be combined with head first!", function () {
      ...
      expect(
        reduce((x, y) => x + y, "", listHelloWorld),
        'Order should be: "" + "hello " + "world" = "hello world"'
      ).to.equal("hello world");
    });
    it("Can reduce to a different type", function () {
      const listNum = cons(3, cons(2, cons(1, null)));
      expect(
        reduce((x, y) => [...x, y], [], listNum),
        "Should put list elements in an array"
      ).to.eql([3, 2, 1]);
    });
  });
  ...
  describe("reduceRight(function, initialValue, list)", function () {
    ...
    it("The order of reduceRight is important. The initial value should be combined with final value!", function () {
      let listHelloWorld = cons("hello ", cons("world", null));
      expect(
        reduceRight((x, y) => x + y, "", listHelloWorld),
        'Order should be: "" + "world " + "hello" = "hello world"'
      ).to.equal("worldhello ");
    });
    it("Can reduce to a different type", function () {
      const listNum = cons(3, cons(2, cons(1, null)));
      expect(
        reduceRight((x, y) => [...x, y], [], listNum),
        "Should reverse list elements in the array"
      ).to.eql([1, 2, 3]);
    });
  });
  ...
  describe("filter(function, list)", function () {
    ...
    it("Filter returns null if returning an empty list", function () {
      ...
      expect(
        newList,
        "expected an empty list to be represented as null"
      ).to.equal(null);
    });
  });
...
describe("exercise_4_suite", function () {
  describe("List<T> class constructor", function () {
    ...
    it("List stores a cons list in this.head property on List", function () {
      expect(
        new List([]).head,
        "Empty list should set this.head to null"
      ).to.equal(null);
      expect(new List([2]).head, "Checking that head is a cons function").is.a(
        "function"
      );
      expect(
        head(new List([2]).head),
        "Checking that head has a value"
      ).to.equal(2);
  ...
  describe("List<T> methods", function () {
    describe("map method", function () {
      it("map method exists on List class", function () {
        expect(
          new List([]),
          "map doesn't exist as a method on the List class"
        ).property("map");
      });
      it("Identity tests", function () {
        expect(new List([]).map((x) => x).toArray()).to.deep.equal(
          new List([]).toArray()
        );
        expect(
          new List([1, 2, 3]).map((x) => x).toArray(),
          "Expect mapping identity to do nothing"
        ).to.deep.equal(new List([1, 2, 3]).toArray());
      });
      it("Transforming number -> string", function () {
        expect(
          new List([1, 2]).map((x) => x.toString()).toArray()
        ).to.deep.equal(new List(["1", "2"]).toArray());
      });
    });
    describe("forEach method", function () {
      it("forEach method exists on List class", function () {
        expect(
          new List([]),
          "forEach doesn't exist as a method on the List class"
        ).property("forEach");
      });
      it("Visits elements and doesn't change list", function () {
        let visited = [];
        let yourList = new List([1, 2, 3, 4]).forEach((el) => {
          visited.push(el);
          return false;
        });
        expect(visited, "Each element wasn't visited").to.deep.equal([
          1, 2, 3, 4,
        ]);
        expect(
          yourList.toArray(),
          "Values returned from the function should be ignored."
        ).to.deep.equal(new List([1, 2, 3, 4]).toArray());
      });
    });
    ...
    describe("filter method", function () {
      it("filter method exists on List class", function () {
        expect(
          new List([]),
          "filter doesn't exist as a method on the List class"
        ).property("filter");
      });
      it("Filters out elements that don't pass the test", function () {
        expect(
          new List([1, 2, 3, 4]).filter((v) => v % 2 == 0).toArray()
        ).to.deep.equal(new List([2, 4]).toArray());
      });
      it("Empty list after filter must be equal to `new List([])`", function () {
        expect(new List([1, 2, 3]).filter(() => false)).to.deep.equal(
          new List([])
        );
      });
    });
    describe("reduce method", function () {
      it("reduce method exists on List class", function () {
        expect(
          new List([]),
          "reduce doesn't exist as a method on the List class"
        ).property("reduce");
      });
      ...
      it("The order of reduce is important. The initial value should be combined with head first!", function () {
        expect(
          new List([1, 2, 3]).reduce((x, y) => x - y, 0),
          "Order should be: ((0 - 1) - 2) - 3 = 6"
        ).to.equal(-6);
      });
  ...
  describe("concat method on List<T> class", function () {
    it("concat method exists on List class", function () {
      expect(
        new List([]),
        "concat doesn't exist as a method on the List class"
      ).property("concat");
    });
    it("new List([1]).concat(new List([2])) equals new List([1,2])", function () {
      expect(new List([1]).concat(new List([2])).toArray()).to.deep.equal(
        new List([1, 2]).toArray()
      );
    });
    it("new List([1,2]).concat(new List([3,4])) equals new List([1,2,3,4])", function () {
      expect(new List([1, 2]).concat(new List([3, 4])).toArray()).to.deep.equal(
        new List([1, 2, 3, 4]).toArray()
      );
    });
    describe("Handling empty list cases", function () {
      it("First list is empty: new List([]).concat(new List([1])) equal to new List([1])", function () {
        expect(new List([]).concat(new List([1])).toArray()).to.deep.equal(
          new List([1]).toArray()
        );
      });
      it("Second list empty: return first list", function () {
        expect(new List([1]).concat(new List([])).toArray()).to.deep.equal(
          new List([1]).toArray()
        );
      });
      it("Both empty lists returns an empty list", function () {
        expect(new List([]).concat(new List([])).toArray()).to.deep.equal(
          new List([]).toArray()
        );
      });
    });
...
describe("exercise_5_suite", function () {
  ...
  describe("b) lineToList(line: [number, string]): List<[number, string]>", function () {
    it("Simple line to list", function () {
      const s = "hey!";
      expect(lineToList(line(s)).toArray()).to.deep.equal(
        new List([[0, s]]).toArray()
      );
    });
  });
});
...
describe("exercise_6_suite", function () {
  ...
  describe("Check that binary tree draws", function () {
    it("Case from worksheet", function () {
      ...
      expect(head(output.head), "First element isn't right").to.deep.equal(
        head(result.head)
      );
      expect(
        output.toArray(),
        "indentation doesn't match what's expected"
      ).to.deep.equal(result.toArray());
    });
  });
});
...
describe("exercise_7_suite", function () {
  ...
  it("Prints the Nary tree in the worksheet", function () {
    ...
    expect(
      output.toArray(),
      `${JSON.stringify(output)} to equal ${JSON.stringify(result)}`
    ).to.deep.equal(result.toArray());
  });
});
