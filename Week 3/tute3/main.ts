...
function numberToString(input: JSON): string {
...
const fromArray = <T>(arr: T[]): ConsList<T> =>
  arr.reduceRight((acc: ConsList<T>, elem: T) => cons(elem, acc), null);
...
//      i.e., f(init, head(l))
const reduce = <X, T>(f: (acc: X, elem: T) => X, init: X, l: ConsList<T>): X =>
  l ? reduce(f, f(init, head(l)), rest(l)) : init;
...
const reduceRight = <X, T>(
  f: (acc: X, elem: T) => X,
  init: X,
  l: ConsList<T>
): X => reduce(f, init, reverse(l));
...
const filter = <T>(f: (elem: T) => boolean, l: ConsList<T>): ConsList<T> =>
  l
    ? f(head(l))
...
const reverse = <T>(l: ConsList<T>): ConsList<T> =>
  l ? concat(reverse(rest(l)), cons(head(l), null)) : null;
...
class List<T> {
  ...
  toArray(): T[] {
    ...
    return reduce((a, t) => a.concat(t), <T[]>[], this.head);
  }
   ...
   */
  reduce<X>(f: (acc: X, elem: T) => X, init: X): X {
...
const line = (s: string): [number, string] => [0, s];
...
const lineToList = (line: [number, string]): List<[number, string]> =>
  new List([line]);
...
const nest = (
  ...
  layout.map((elem: [number, string]) => [elem[0] + indent, elem[1]]);
...
function prettyPrintNaryTree<T>(node: NaryTree<T>): List<[number, string]> {
  return node
    ? lineToList(line(node.data.toString())).concat(
        nest(
          1,
          node.children.reduce(
            (a, b) => a.concat(prettyPrintNaryTree(b)),
            new List([])
          )
        )
      )
 ...
 */
// This is extra stuff so feel free to understand on your own. Not gonna bother explaining
// A chain my methods into a single line of code hence why the weird looking code
type jsonTypes =
  | Array<jsonTypes>
  ...
  if (Array.isArray(json)) {
      ...
      .concat(
        nest(
          ...
          json
            ...
            .split(" ")
            .reduce(
              (a, b) => a.concat(jsonPrettyToDoc(b)),
              new List<[number, string]>([])
            )
        )
      )
    ...
    return lineToList(line("{"))
      .concat(
        nest(
          1,
          new List(Object.keys(json)).reduce(
            (acc, x) =>
              x != ","
                ? acc
                    .concat(jsonPrettyToDoc(x + ":"))
                    .concat(jsonPrettyToDoc(json[x]))
                : acc.concat(jsonPrettyToDoc(x)),
            new List<[number, string]>([])
          )
        )
      )
...
console.log(
  jsonPrettyToDoc(json).map(lineIndented).reduce(appendLine, "").trim()
);
